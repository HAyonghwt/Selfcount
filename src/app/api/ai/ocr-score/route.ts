...
export async function POST(req: NextRequest) {
    try {
      ...
      4. [논리적 검증] 각 선수의 1홀부터 9홀까지 점수의 합계가 'Total' 칸에 적힌 숫자와 일치하는지 반드시 확인하세요. 
         만약 불일치한다면, 필기 숫자를 다시 한번 정밀하게 분석하여 가장 논리적으로 타당한 결과를 도출하세요.
      5. 만약 특정 홀에 점수가 적혀 있지 않다면 null로 표시하세요.
      6. [합계 처리] 합계(Total) 칸의 숫자를 반드시 읽으세요. 읽을 수 없거나 칸이 비어있는 경우에만 개별 홀의 합산값을 사용하되, 가급적 수기된 숫자를 우선적으로 인식하세요.
