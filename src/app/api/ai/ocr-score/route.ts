...
export async function POST(req: NextRequest) {
    try {
      ...
      5. [매우 중요] 각 선수의 합계(Total) 칸에 적힌 숫자를 반드시 읽어주세요. 합계 칸이 있다면 절대 빠뜨리지 말고 숫자로 반환하세요. 합계 칸이 없거나 읽을 수 없으면 null로 표시하세요. 서명(Signature) 칸은 무시하세요.
      6. 이름 칸이 상하 두 줄로 나뉘어 적힌 경우(예: 위에 성, 아래에 이름) 이를 하나로 합쳐서 하나의 선수 이름으로 인식하세요. (예: '고'와 '기영'이 상하로 적혀 있으면 '고기영'으로 인식)
      7. 모든 이름과 숫자를 정확하게 읽어주세요.
    `;
